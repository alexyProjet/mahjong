            ...
            <div id="Rules">
                ...
                <select class="lobbyParameter" id="ruleInput">
                    <c:forEach items="${rules}" var="rule" varStatus="status">
                        <option ${selectedRule==rule ? "selected": ""}>${rule.getName()}</option>
                    </c:forEach>
        ...
        <script>
            ...
            launchButton.addEventListener("click", function () {
                ...
                if (name) {
                    ...
                    let url = '/createLobby?name=' + name + "&visibility=" + visibility + "&rule=" + rule + "&playTime=" + playTime + "&stealTime=" + stealTime + "&playerName=" + localStorage.getItem("playerName");
                    Http.open("POST", url, true);
                    Http.onreadystatechange = (e) => {
                        window.location.href = "/lobby?lobbyId=" + e.currentTarget.response.substring(0, 36) + "&playerId=" + e.currentTarget.response.substring(37, 73);
                    };
                    Http.send();
                } else
